...
5. **Result visualization — DECIDED:** Chart type follows dataset shape, then the proposed insight. Noul/Score on a sequential play-state table render a live scrubbable series over play/row index (sample win likelihood is Noul P(win) 0–1, not CSV `wpa`; play quality / grading is Score over the same play index, not Choice Good/Bad on H1). Place/eating tables (squirrel census) render a **labeled eating map with counts** when coordinates exist, plus **ranked-eating bars**; never Location vs Activity Choice bars or a P(moving) series over sighting index. Choice classifiers render class-distribution bars when that is actually the insight. The `Row X of Y` rail is only for row-streamed series/bars. Progress percent chrome stays. Empty series/bars: chart axes + “Waiting for the first row…”; places can plot dataset coordinates immediately.
6. **Sample datasets — DECIDED:** Fixtures stay as raw CSV assets and `/dataset/:id` routes, not landing CTAs. Seahawks fixture is the Super Bowl demo CSV (`public/samples/seahawks-super-bowl-2026.csv`; win likelihood → P(win) line). **Squirrel census** is the second fixture (`public/samples/nyc-squirrel-census.csv`; slim checked-in table exercising place/eating/lat-lng). Default squirrel insights are “Where are they eating?” (labeled map) and “Which places have the most eating?” (ranked bars). Landing idle is BYOD only (upload + public HTTPS CSV URL). Do not drop the football fixture while keeping the second sample.
