...
export const SQUIRREL_ACTIVITY_TASK = 'Is this squirrel on the move given this sighting?'
export const SQUIRREL_ACTIVITY_QUERY = 'Is this squirrel on the move given this sighting?'
...
const PLACE_EATING_RE = /where\s+(?:are\s+they|they)\s+eat|which\s+places?\s+have\s+the\s+most\s+eating|locations?\s+where\s+(?:\w+\s+)?(?:squirrels?|they)\s+(?:are\s+)?(?:spotted\s+)?eat|spotted\s+eating|eating\s+locations?|places?\s+(?:they|squirrels?)\s+eat/i
